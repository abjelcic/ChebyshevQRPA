...
C     ================================================================
     ...
     &       iaIPP(IP_NCF),iaINN(IP_NCF)
      ...
      call dumpmatd( iNV , iNV , dpaA1 , IP_NCF , './out_matvec/A.mat');
      call dumpmatd( iNV , iNV , dpaB1 , IP_NCF , './out_matvec/B.mat');
#endif
...
#ifndef ORIGINAL_CODE
      double precision fIS_mn_r( IP_NCF , IP_NNP ); ! INV1xiNMAX size matrix, operator f for prot+neut, first index (m,n) pairs, second radial coordinate
      double precision fIS_mn  ( IP_NCF          ); ! INV1       size vector, operator f for prot+neut, index (m,n) pairs
      double precision fIV_mn_r( IP_NCF , IP_NNP ); ! INV1xiNMAX size matrix, operator f for prot-neut, first index (m,n) pairs, second radial coordinate
      double precision fIV_mn  ( IP_NCF          ); ! INV1       size vector, operator f for prot-neut, index (m,n) pairs
#endif
...
#ifndef ORIGINAL_CODE
      do i = 1 , iNMAX ! radial index
          do i2 = 1 , iNV1 ! (m,n) pairs index

              ! calculating fIS_mn_r(i2,i) and fIV_mn_r(i2,i)
              fIS_mn_r(i2,i) = 0.d0;
              fIV_mn_r(i2,i) = 0.d0;

              iQ2 = iaIQ(iaIPP(iaNUM(i2))) + iaIQ(iaINN(iaNUM(i2)));
              if( iQ2.ne.0 .and. IQ2.ne.2 ) then
                  stop 'IQ2 should be 0 or 2 in ELECTRO()!';
              endif

              if( iaJJ(iaNUM(i2)) .ne. iORB ) then
                  stop 'iaJJ not equal to iORB=J in ELECTRO()!';
              endif


              iISOSPIN = 0;
              if( iQ2 .eq. 2 ) iISOSPIN = +1;
              if( iQ2 .eq. 0 ) iISOSPIN = -1;

              iEXP = ( - iaLL(iaIPP(iaNUM(i2)))
     &                 + iaLL(iaINN(iaNUM(i2)))
     &                 + iORB                   ) / 2;
              iSGN = if_POT(iEXP);


              fIS_mn_r(i2,i) = iSGN * 1
     &                * dpaRMAT1(1,iaIPP(iaNUM(i2)),iaINN(iaNUM(i2)))
     &                * dpaWF(i,iaIPP(iaNUM(i2)))
     &                * dpaWF(i,iaINN(iaNUM(i2)))
     &                / ((dfloat(i)*dpStep)**2);

              fIV_mn_r(i2,i) = iSGN * iISOSPIN
     &                * dpaRMAT1(1,iaIPP(iaNUM(i2)),iaINN(iaNUM(i2)))
     &                * dpaWF(i,iaIPP(iaNUM(i2)))
     &                * dpaWF(i,iaINN(iaNUM(i2)))
     &                / ((dfloat(i)*dpStep)**2);
              ...
              fIS_mn_r(i2,i) = fIS_mn_r(i2,i)
     &                * (-1.d0)/dsqrt(dfloat(2*iISPIN+1));

              fIV_mn_r(i2,i) = fIV_mn_r(i2,i)
     &                * (-1.d0)/dsqrt(dfloat(2*iISPIN+1));
      ...
      if( iRAD .ne. 1 ) then
          stop 'iRAD must be 1, change line 8 in skyrme_rpa.in file';
      endif

      do i2 = 1 , iNV1 ! (m,n) pairs index

          ! calculating fIS_mn(i2) and fIV_mn(i2)
          fIS_mn(i2) = 0.d0;
          fIV_mn(i2) = 0.d0;
          ...
          do i = 1 , iNMAX ! radial index
              ...
              fIS_mn(i2) = + fIS_mn(i2)
     &                     + fIS_mn_r(i2,i)*dpXOP*dpStep;

              fIV_mn(i2) = + fIV_mn(i2)
     &                     + fIV_mn_r(i2,i)*dpXOP*dpStep;
          end do

          fIS_mn(i2) = fIS_mn(i2) * dsqrt(dble(2*iISPIN+1));
          fIV_mn(i2) = fIV_mn(i2) * dsqrt(dble(2*iISPIN+1));

      end do

      call dumpvecd( iNV1 , fIS_mn , './out_matvec/F20_ISOSCALAR.vec' );
      call dumpvecd( iNV1 , fIS_mn , './out_matvec/F02_ISOSCALAR.vec' );
      call dumpvecd( iNV1 , fIV_mn , './out_matvec/F20_ISOVECTOR.vec' );
      call dumpvecd( iNV1 , fIV_mn , './out_matvec/F02_ISOVECTOR.vec' );
#endif
...
C     IV dipole
     ...
     &             dpaBEL_IV(i),dpaFRACTS0_IV(i)
      ...
      ! bug fixed
      sFileName='Plot_Bel_IS.dat'
      call AVERLOR(sFileName,dpaFreq,dpaBEL_IS,iNV1)
      sFileName='Plot_Bel_EM.dat'
      call AVERLOR(sFileName,dpaFreq,dpaBelElec,iNV1)
      sFileName='Plot_Bel_IV.dat'
      call AVERLOR(sFileName,dpaFreq,dpaBEL_IV,iNV1)
#endif
      write(2,*)''
...
C     ================================================================
...
#else
      parameter( i_MAX_OMEGA_POINTS = 100000000 );
      dimension dpaSaf(i_MAX_OMEGA_POINTS),dpaEx1(i_MAX_OMEGA_POINTS);
      if( iP_NS .gt. i_MAX_OMEGA_POINTS ) then
          stop 'i_MAX_OMEGA_POINTS too small in AVERLOR()';
      endif
#endif
 ...
 100  format(1X,5(E12.6,1X))
      ...
      subroutine dumpmatd( m , n , A , LDA , outfile )
      ...
      open(newunit=tape, file = outfile , status="unknown" );

      write(tape,'(a)') '#Number of rows';
      write(tape,'(i8)') m;
      write(tape,'(a)') '#Number of columns';
      write(tape,'(i8)') n;

      do i = 1 , m
          do j = 1 , n
              write(tape,'(i8,i8,E30.14)') i , j , A(i,j);
          enddo
      enddo
      ...
      subroutine dumpvecd( n , V , outfile )
      ...
      write(tape,'(a)') '#Number of elements';
      write(tape,'(i8)') n;

      do i = 1 , n
          write(tape,'(i8,E30.14)') i , V(i);
      enddo
